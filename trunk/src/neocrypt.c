...
{
    int i;
    int rom_size = memory.gfx_size;
    int tx_size = memory.sfix_size;
    Uint8 *src;
	if (!memory.mapped) src = memory.gfx + rom_size - tx_size;
	else src = (Uint8*)(rom_size - tx_size);
    Uint8 *dst = memory.sfix_game;
	int temp = 0;

    for (i = 0; i < tx_size; i++) {
		if (memory.mapped) {
			temp = ((int)src) + (i & ~0x1f) + ((i & 7) << 2) + ((~i & 8) >> 2) + ((i & 0x10) >> 4);
			dst[i] = read_byte_file(temp);
			if (i%400==0) update_progress_bar(i,tx_size);
		} else {
