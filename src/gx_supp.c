...
/****************************************************************************
* GX_Render For Buffer with Viewport
*
* Pass in a buffer to update as a tiled RGB565 texture
****************************************************************************/
void GX_Render2(t_bitmap *buffer) {
  int h, w;
  ...
  u64 *src1; u64 *src2; u64 *src3; u64 *src4;
	...
	GX_SetTevOrder(GX_TEVSTAGE0, GX_TEXCOORD0, GX_TEXMAP0, GX_COLOR0A0);
		...
		for (h = 0; h < vheight; h += 4) {
  		src1 = (u64 *)( buffer->data + (buffer->viewport.x*buffer->granularity) + ((buffer->viewport.y + h) * buffer->pitch) );
  		src2 = src1 + (buffer->width>>2);
  		src3 = src2 + (buffer->width>>2);
  		src4 = src3 + (buffer->width>>2);
